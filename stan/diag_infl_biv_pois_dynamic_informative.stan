...
data{
  ...
  // per-team informative priors
  int<lower=0, upper=1> ind_inf_prior;
  vector[nteams] prior_att_mean;
  vector<lower=0>[nteams] prior_att_sd;
  vector[nteams] prior_def_mean;
  vector<lower=0>[nteams] prior_def_sd;
...
model{
  ...
  // ========================================
    ...
    for (h in 1:nteams) {
      for (i in 1:ntimes) {
        if (i == 1 && ind_inf_prior == 1) {
          target += normal_lpdf(att_raw[1,h] | prior_att_mean[h], prior_att_sd[h]);
          target += normal_lpdf(def_raw[1,h] | prior_def_mean[h], prior_def_sd[h]);
        }
        else if (prior_dist_num == 1) {
